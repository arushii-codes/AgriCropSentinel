  ...
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      return const InsightCard(
        ...
        title: 'Wheat Price',
        subtitle: 'Loading market prediction...',
        trailing: SizedBox(
          width: 20,
          height: 20,
    ...
    if (snapshot.hasError || !snapshot.hasData) {
      return const InsightCard(
        imagePath: 'assets/soybean.png',
        title: 'Wheat Price',
        subtitle: 'Prediction unavailable',
        trailing: Icon(
          Icons.error_outline,
    ...
    final predictionData =
        snapshot.data!['prediction_data'] as Map<String, dynamic>;

    final currentPrice = predictionData['current_price'];
    final trend = predictionData['trend'] ?? 'Stable';
    final confidence =
        ((predictionData['confidence'] ?? 0) * 100).round();

    final isRising = trend.toString().toLowerCase() == 'rising';

    return InsightCard(
      imagePath: 'assets/soybean.png',
      title: 'Wheat Price',
      subtitle:
          '₹$currentPrice/quintal • $confidence% confidence',
      trailing: Text(
